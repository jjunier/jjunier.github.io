...
## 이번 주 학습 목표 
---
- dbt의 핵심 개념과 **구성 요소(models, materialization, tests, snapshots, docs)**를 이해하고, **ELT 기반 데이터 파이프라인을 구조적으로 설계**할 수 있다.
- Fact·Dimension 모델링, 증분 로드, 변경 이력 관리(SCD Type 2), 데이터 품질 테스트를 통해 **신뢰성 있는 분석용 데이터셋을 구축**할 수 있다.
- dbt 문서화와 데이터 카탈로그 개념을 활용해 **데이터 리니지, 메타데이터, 거버넌스를 체계적으로 관리하는 방법을 이해**한다.
